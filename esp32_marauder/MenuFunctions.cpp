  ...
  // Screen blackout: wake on any tap → restore last saved brightness
    ...
    if (pressed && wifi_scan_obj.currentScanMode != WIFI_SCAN_OFF &&
      ...
      if (t_y < zoneUp) {
        ...
        // Always consume touch in top zone during scan — don't fall through to stop scan
        return;
      }
    }
  #endif
...
// Hold top/bottom 2.5s to enter. TAP TOP = brighter, TAP BOTTOM = dimmer.
...
void MenuFunctions::drawAutoCycleStatus() {
  ...
  // Layout constants
  const uint16_t modeY = 50;
  const uint16_t barY = 90;
  const uint16_t barH = 20;
  const uint16_t barX = 20;
  const uint16_t barW = TFT_WIDTH - 40;
  const uint16_t timeY = 120;
  const uint16_t stepY = 155;
  const uint16_t cycleY = 185;
  const uint16_t stopY = 240;
